...
public class CsTaskServiceImpl implements ICsTaskService {
    ...
    public ResultVo<ResultReasonVo> load(LoadTaskDto paramsDto) {
        ...
        for (Long taskCarId : paramsDto.getTaskCarIdList()) {
            ...
            //运单和车辆状态
            int waybillCarNewState = WaybillCarStateEnum.WAIT_LOAD_TURN.code;
            int orderCarNewState = orderCar.getState();
            if (waybill.getType() == WaybillTypeEnum.PICK.code && waybillCar.getStartStoreId() == null) {
                //提车任务、自送到业务中心
                waybillCarNewState =  WaybillCarStateEnum.LOADED.code;
                orderCarNewState = OrderCarStateEnum.PICKING.code;
            } else if (waybill.getType() == WaybillTypeEnum.TRUNK.code && waybillCar.getStartStoreId() == null) {
                //干线、提干、干送、提干送任务
                waybillCarNewState = WaybillCarStateEnum.LOADED.code;
                orderCarNewState = OrderCarStateEnum.TRUNKING.code;
            } else if (waybill.getType() == WaybillTypeEnum.BACK.code && waybillCar.getStartStoreId() == null) {
                //送车、自提
                waybillCarNewState = WaybillCarStateEnum.LOADED.code;
            ...
            if(orderCar.getState() < orderCarNewState){
                ...
                //车辆轨迹
                csOrderCarLogService.asyncSave(Lists.newArrayList(orderCar),
                        OrderCarLogEnum.IN_STORE,
                        new Object[]{OrderCarLogEnum.IN_STORE.getInnerLog(), OrderCarLogEnum.IN_STORE.getOutterLog()}, userInfo);
            }
