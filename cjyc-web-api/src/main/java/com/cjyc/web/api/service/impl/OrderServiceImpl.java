...
public class OrderServiceImpl extends ServiceImpl<IOrderDao, Order> implements IOrderService {
    ...
    public void importCustomerOrder(List<ImportCustomerOrderDto> orderList,
        ...
        if (!CollectionUtils.isEmpty(orderList) && !CollectionUtils.isEmpty(carList)) {
            ...
            //生成订单号并批量保存
            orderMap.forEach(new BiConsumer<Integer, ImportCustomerOrderDto>() {
                @Override
                public void accept(Integer integer, ImportCustomerOrderDto dto) {
                    String orderNo = csSendNoService.getNo(SendNoTypeEnum.ORDER);
                    List<ImportCustomerOrderCarDto> carList = carMap.get(dto.getOrderNo());
                    if (!CollectionUtils.isEmpty(carList)) {
                        //保存订单、车辆信息
                        BigDecimal totalFee = new BigDecimal(0);
                        for (ImportCustomerOrderCarDto c: carList) {
                            totalFee = totalFee.add(c.getPickFee())
                                    .add(c.getSendFee()).add(c.getWlFee());
                        }
                        Order order = packCustomerOrderForImport(dto, orderNo,
                                admin, carList.size(), totalFee);
    ...
    public void importPatCustomerOrder(List<ImportPatCustomerOrderDto> orderList,
        ...
        if (!CollectionUtils.isEmpty(orderList) && !CollectionUtils.isEmpty(carList)) {
            ...
            //生成订单号并批量保存
            orderMap.forEach(new BiConsumer<Integer, ImportPatCustomerOrderDto>() {
                @Override
                public void accept(Integer integer, ImportPatCustomerOrderDto dto) {
                    //保存合伙人及车辆信息
                    String orderNo = csSendNoService.getNo(SendNoTypeEnum.ORDER);
                    List<ImportPatCustomerOrderCarDto> carList = carMap.get(dto.getOrderNo());
                    if (!CollectionUtils.isEmpty(carList)) {
                        BigDecimal totalFee = new BigDecimal(0);
                        for (ImportPatCustomerOrderCarDto c: carList) {
                            c.setPickFee(c.getPickFee()==null?new BigDecimal(0): c.getPickFee());
                            c.setSendFee(c.getSendFee()==null?new BigDecimal(0): c.getSendFee());
                            totalFee = totalFee.add(c.getPickFee())
                                    .add(c.getSendFee()).add(c.getWlFee());
                        }
                        Order order = packPatCustomerOrderForImport(dto, orderNo, admin,
                                carList.size(), totalFee);
                        orderDao.insert(order);
                        Long orderId = order.getId();
                        for (int i = 1; i < carList.size() + 1; i++) {
                            String carNo = csSendNoService.formatNo(orderNo, i, 3);
